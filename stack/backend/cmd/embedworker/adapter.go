...
// The adapters below bridge the broker's transport types to the worker's
// transport-free interfaces, so internal/embedjob never imports internal/queue.
// They are thin delegations; the worker's broker round-trip is covered by the
// integration test in this package.

// qDelivery adapts a queue.Delivery to embedjob.Delivery.
...
// qStream adapts a queue.Client's consume stream to embedjob.Stream, wrapping
...
func (s qStream) Consume(ctx context.Context) (<-chan embedjob.Delivery, error) {
...
// qEnqueuer adapts a queue.Client to embedjob.Enqueuer for bounded retries.
