...
// transport-free interfaces, so internal/evidencejob never imports internal/queue.

// qDelivery adapts a queue.Delivery to evidencejob.Delivery.
...
// qStream adapts a queue.Client's consume stream to evidencejob.Stream, wrapping
...
func (s qStream) Consume(ctx context.Context) (<-chan evidencejob.Delivery, error) {
	raw, err := s.client.Consume(ctx)
	if err != nil {
		return nil, err
	}
	out := make(chan evidencejob.Delivery)
	go func() {
		defer close(out)
		for d := range raw {
			select {
...
// qEnqueuer adapts a queue.Client to evidencejob.Enqueuer for bounded retries.
