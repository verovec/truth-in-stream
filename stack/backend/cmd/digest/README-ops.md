# digest - daily development digest

`digest` assembles a summary of the last 24 hours of development - git commits,
Linear card activity, open GitHub pull requests, and stalled In Progress cards -
and posts it to Slack as a Block Kit message, or prints it to the terminal.

It runs independently of the API server: a cron job (or any scheduler) invokes
the binary once a day. There is no long-running process.

## Modes

| Command | Effect |
|---------|--------|
| `go run ./cmd/digest` | Post the Block Kit digest to Slack (default). |
...
| `LINEAR_API_KEY` | No | Enables the Linear activity and blocker sections. Absent -> those sections show a note. |
