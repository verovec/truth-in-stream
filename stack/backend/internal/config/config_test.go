...
func TestLoadWikiEmbed(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "defaults applied",
			env:  map[string]string{},
			want: WikiEmbed{BatchSize: 128, Concurrency: 4, MaxRetries: 6, HTTPTimeout: 120 * time.Second, MaintenanceWorkMem: "512MB", MaxParallelWorkers: 7},
		},
		{
			...
			want: WikiEmbed{BatchSize: 256, Concurrency: 8, MaxRetries: 3, HTTPTimeout: 90 * time.Second, MaintenanceWorkMem: "2GB", MaxParallelWorkers: 4},
		},
