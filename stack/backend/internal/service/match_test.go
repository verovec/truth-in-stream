...
func testMatcherConfig() MatcherConfig {
	return MatcherConfig{TopK: 3, ScoreThreshold: 0.5, EmbedConcurrency: 4, Timeout: time.Minute}
}
...
func TestMatchSegment(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			m, err := NewMatcher(tc.embedder, tc.searcher, tc.cfg)
			if err != nil {
...
func TestMatchSegmentRejectsDimensionMismatchBeforeSearch(t *testing.T) {
	t.Parallel()
	searcher := &fakeSearcher{}
	m, err := NewMatcher(&fakeEmbedder{vecs: [][]float32{{1, 2, 3}}}, searcher, testMatcherConfig())
	if err != nil {
		t.Fatalf("NewMatcher: %v", err)
	}
...
func TestNewMatcherValidatesConfig(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg := testMatcherConfig()
			tc.mutate(&cfg)
			_, err := NewMatcher(&fakeEmbedder{}, &fakeSearcher{}, cfg)
...
func TestMatcherBoundsEmbedConcurrency(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		m, err := NewMatcher(embedder, stubSearcher{}, cfg)
		if err != nil {
			t.Fatalf("NewMatcher: %v", err)
		}
