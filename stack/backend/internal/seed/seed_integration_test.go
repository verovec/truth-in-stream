...
func resetSchema(ctx context.Context, t *testing.T, dsn string) {
	...
	if _, err := pool.Exec(ctx, "DROP TABLE IF EXISTS claims, documents, segment_results, processed_videos, videos, wiki_chunks, wiki_chunks_staging, wiki_chunks_old, wiki_sync_state, political_claims, voting_records"); err != nil {
