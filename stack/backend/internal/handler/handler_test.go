...
// testTVHub builds an inert TV hub over the stub analyzer for wiring NewMux in
// tests that do not exercise the hub. Its Live always reports false, so the
// channel list renders channels as off air.
func testTVHub() *service.TVHub {
	hub, err := service.NewTVHub(stubLiveAnalyzer{}, slog.New(slog.NewJSONHandler(io.Discard, nil)))
	if err != nil {
		panic(err)
	}
	return hub
...
func newAuthedTestServer(auth AuthConfig, storeErr error) http.Handler {
	...
	return NewMux(health, &fakeVideoService{}, &fakeVideoAnalysisService{}, &fakeDocumentService{}, &fakeDocumentAnalyzer{}, &fakeYouTubeService{}, &fakeTVChannelService{}, &fakeTVRecordingService{}, testTVHub(), stubLiveAnalyzer{}, nil, nil, nil, false, nil, "", auth, logger)
}
...
func newDebugTestServer() http.Handler {
	...
	logger := slog.New(slog.NewJSONHandler(io.Discard, nil))
	return NewMux(health, &fakeVideoService{}, &fakeVideoAnalysisService{}, &fakeDocumentService{}, &fakeDocumentAnalyzer{}, &fakeYouTubeService{}, &fakeTVChannelService{}, &fakeTVRecordingService{}, testTVHub(), stubLiveAnalyzer{}, nil, nil, nil, false, &fakeWikiSearcher{}, "", globalTestAuth, logger)
}
