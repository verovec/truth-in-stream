...
func newAuthedTestServer(auth AuthConfig, storeErr error) http.Handler {
	...
	return NewMux(health, &fakeVideoService{}, &fakeYouTubeService{}, stubLiveAnalyzer{}, nil, nil, false, nil, "", auth, logger)
}
...
func newDebugTestServer() http.Handler {
	...
	logger := slog.New(slog.NewJSONHandler(io.Discard, nil))
	return NewMux(health, &fakeVideoService{}, &fakeYouTubeService{}, stubLiveAnalyzer{}, nil, nil, false, &fakeWikiSearcher{}, "", globalTestAuth, logger)
}
