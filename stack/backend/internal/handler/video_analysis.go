...
// Durable video analysis read API.
//
//	GET /api/videos/{id}/analysis   the video's pre-analysis lifecycle and,
//	                                when complete, the stored frame list
//
// The endpoint is the poll target while a pre-analysis runs and the hydration
// source once it completes: frames carries the whole session in the exact wire
// shapes the live WebSocket emits (shared serializer, see toLiveFrame), with
// absolute video-time timestamps, so the frontend replays it through its
// existing frame reducers without a socket or audio capture.
