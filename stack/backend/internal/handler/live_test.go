...
func liveMuxServer(t *testing.T, analyzer LiveAnalyzer, debugFactCheck bool) string {
	...
	mux := NewMux(hc, &fakeVideoService{}, &fakeDocumentService{}, &fakeDocumentAnalyzer{}, &fakeYouTubeService{}, &fakeTVChannelService{}, analyzer, nil, nil, nil, debugFactCheck, nil, "", globalTestAuth, logger)
	srv := httptest.NewServer(mux)
	t.Cleanup(srv.Close)
	return "ws" + strings.TrimPrefix(srv.URL, "http") + "/api/videos/vid1/live"
}
