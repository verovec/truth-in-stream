...
func tvHubMuxServer(t *testing.T) string {
	...
	mux := NewMux(hc, &fakeVideoService{}, &fakeDocumentService{}, &fakeDocumentAnalyzer{}, &fakeYouTubeService{}, &fakeTVChannelService{}, testTVHub(), stubLiveAnalyzer{}, nil, nil, nil, false, nil, "", globalTestAuth, logger)
	srv := httptest.NewServer(mux)
	t.Cleanup(srv.Close)
	return "ws" + strings.TrimPrefix(srv.URL, "http")
}
