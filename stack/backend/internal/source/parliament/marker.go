...
// marker is the persisted conditional-GET validators for a dataset's bulk dump.
...
func loadMarker(path string) (marker, error) {
	...
	if err != nil {
		...
		return marker{}, fmt.Errorf("parliament: read marker %q: %w", path, err)
	}
	var m marker
	if err := json.Unmarshal(data, &m); err != nil {
		return marker{}, fmt.Errorf("parliament: decode marker %q: %w", path, err)
	}
	return m, nil
}

// saveMarker writes the validators to path atomically (write to a temp file in the
...
func saveMarker(path string, m marker) error {
	...
	if err != nil {
		return fmt.Errorf("parliament: encode marker: %w", err)
	}
	dir := filepath.Dir(path)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("parliament: create marker dir %q: %w", dir, err)
	}
	tmp, err := os.CreateTemp(dir, ".marker-*.tmp")
	if err != nil {
		return fmt.Errorf("parliament: create temp marker: %w", err)
	}
	tmpName := tmp.Name()
	defer func() { _ = os.Remove(tmpName) }()
	if _, err := tmp.Write(data); err != nil {
		_ = tmp.Close()
		return fmt.Errorf("parliament: write temp marker: %w", err)
	}
	if err := tmp.Close(); err != nil {
		return fmt.Errorf("parliament: close temp marker: %w", err)
	}
	if err := os.Rename(tmpName, path); err != nil {
		return fmt.Errorf("parliament: replace marker %q: %w", path, err)
	}
	return nil
}
