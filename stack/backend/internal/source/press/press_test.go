...
func TestKind(t *testing.T) {
	...
	if p.Kind() != source.KindAttribution {
		t.Fatalf("Kind: got %q want %q", p.Kind(), source.KindAttribution)
	}
}

func TestRetrieveQuotePassages(t *testing.T) {
	t.Parallel()
	fixture := readFixture(t, "brave_attribution_fr.json")
	...
	ev, err := pack.Retrieve(t.Context(), source.Query{Text: `Marine Tournier "il faut doubler le budget de la defense"`})
	if err != nil {
	...
	first := ev[0]
	// Source carries the press outlet name (not a generic family) and a date.
	if first.Source.Name != "Le Monde" {
		t.Errorf("source name: got %q want Le Monde", first.Source.Name)
	}
	if first.Source.URL != "https://www.lemonde.fr/politique/article/2024/03/12/tournier-defense.html" {
		t.Errorf("source url: got %q", first.Source.URL)
	}
	// page_age (full timestamp) is preferred over the coarse age when present.
	if first.Source.Date != "2024-03-12T18:30:00" {
		t.Errorf("source date: got %q want page_age", first.Source.Date)
	}
	if !strings.Contains(first.Passage, "doubler le budget de la defense") {
		t.Errorf("passage missing the quote:\n%s", first.Passage)
	}
	if !strings.Contains(first.Passage, "sous-investir dans son armee") {
		t.Errorf("passage missing extra snippet:\n%s", first.Passage)
	}

	// Second result falls back to the coarse age when no page_age is present.
	if ev[1].Source.Name != "Le Figaro" {
		t.Errorf("second source name: got %q", ev[1].Source.Name)
	}
	if ev[1].Source.Date != "2024-03-13" {
		t.Errorf("second source date: got %q want age fallback", ev[1].Source.Date)
	}
	...
	if first.ID.Kind != source.KindAttribution || first.ID.SourceID != "www.lemonde.fr" {
