...
# One route table per private subnet so per-AZ NAT works when enabled; with a
# single NAT every table points at NAT 0.
...
# Keeps S3 traffic (ECR layers, backups) off the NAT path.
