...
# access). Value set out of band via `make push-secrets`, never in Terraform.
# Created only when Keycloak is self-hosted here.
resource "aws_secretsmanager_secret" "keycloak_bootstrap_admin_password" {
  ...
  description = "Keycloak bootstrap admin password. Value set manually, never in Terraform."
}

# Password for the scoped `keycloak` Postgres role. The container lives here; the
# value is generated and written by the keycloak DB-bootstrap task (VER-157), the
# single place that both creates the role and hands Keycloak its password.
resource "aws_secretsmanager_secret" "keycloak_db_password" {
  count       = local.keycloak_enabled ? 1 : 0
  name        = "${local.project}/${var.environment}/keycloak/db-password"
  description = "Password for the scoped keycloak Postgres role. Generated by the DB-bootstrap task, never committed."
}
