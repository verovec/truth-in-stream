...
| VER-74 | EPIC: Fact-checkable crawl ingestion (API crawl + LLM gate + AWS mirror) | Todo | High |  |
...
VER-79 -> VER-80
(VER-74 is a tracking EPIC; its child cards VER-75,76,78,79,80 are all delivered or ready)

## Ready queue

Computed: Todo cards whose every depends_on is Done (or one In Review), ordered by priority, unblock-count, number.

1. VER-80 - Cloud/AWS wiring for the crawl pipeline (Medium) - depends_on VER-79 [Done] -> READY off main

Not pickable: VER-74 (tracking EPIC, "do not pick").
